...
#if __has_include("ygopro-core/common.h")
...
#if __has_include("ygopro-core/card_data.h")
#include "ygopro-core/card_data.h"
#elif __has_include("card_data.h")
#include "card_data.h"
#elif __has_include("ygopro-core/card.h")
...
namespace ygopro
{
  // Compatibility aliases for newer core/header variants.
...
#define MAX_REPLAY_SIZE 0x20000
  ...
  public:
    ...
    std::string verbose_str_ = ""; // <-- ADD THIS LINE
  ...
  inline card_data db_query_card_data(const SQLite::Database &db, CardCode code)
  {
    ...
    // Compatible with core variants where `set_setcode` may not exist.
    {
      int ctr = 0;
      while (setcode)
      {
        if (setcode & 0xffff)
        {
          card.setcode[ctr] = setcode & 0xffff;
          ++ctr;
          if (ctr >= SIZE_SETCODE)
          {
            break;
  ...
  inline bool c_has_card(CardCode code)
  {
  ...
  inline bool c_has_card_id(CardCode code)
  {
    return card_ids_.find(code) != card_ids_.end();
  }

  inline bool maybe_set_action_cid(LegalAction &la, CardCode code, const char *ctx,
                                   int msg, uint32_t desc)
  {
    if (code == 0)
    {
      return false;
    }
    if (!c_has_card_id(code))
    {
      fmt::println(stderr,
                   "[ygoenv-desc] skip unknown code in {} msg={} ({}) desc={} code={}",
                   ctx, msg, msg_to_string(msg), desc, code);
      return false;
    }
  ...
  inline std::string describe_effect_label(CardCode code, int eff_idx)
  {
    if (code == 0)
    {
      return get_system_string(eff_idx);
    }
    if (!c_has_card(code))
    {
      return fmt::format("Unknown effect code {} ({})", code, eff_idx);
    }
    const Card &c = c_get_card(code);
    return fmt::format("{} ({})", c.name(), c.get_effect_description(code, eff_idx));
  }
  ...
  public:
    ...
    void WriteState(State &state)
    {
      ...
      auto &actions_str_arr = state["info:legal_actions_str"_];
      actions_str_arr.Zero(); // Ensure the buffer is clean

      if (n_options > 0)
      {
        // Use the existing fmt::formatter to create the desired string
        std::string actions_str = fmt::format("{}", legal_actions_);

        // Cast the void* to char*
        char *data_buffer = static_cast<char *>(actions_str_arr.Data());
        int buffer_size = actions_str_arr.Shape()[0];

        // Copy the string into the char buffer
        strncpy(data_buffer, actions_str.c_str(), buffer_size - 1);
  ...
  private:
    ...
    void _set_obs_action_effect(TArray<uint8_t> &feat, int i, int effect)
    {
      ...
      {
        // Core may pass raw !system N ids (strings.conf) not present in system_string2id;
        // expanding that table reshuffles all encoded ids (make_ids is order-based).
        const auto sit = system_string2id.find(effect);
        if (sit != system_string2id.end())
        {
          effect = sit->second;
        }
        else
        {
          effect = static_cast<int>(std::min(std::max(effect, 0), 255));
        }
      }
    ...
    void handle_message()
    {
      ...
      else if (msg_ == MSG_MOVE)
      {
        ...
        auto getspec = [&](auto &p)
        { return p.get() == pl.get() ? plspec : opspec; };
      ...
      else if (msg_ == MSG_SELECT_BATTLECMD)
      {
        ...
        for (const auto [code_t, spec, desc] : activatable)
        {
          ...
          maybe_set_action_cid(la, code_d, "select_battlecmd", msg_, desc);
      ...
      else if (msg_ == MSG_SELECT_CARD)
      {
        ...
        if (verbose_)
        {
          ...
          for (int i = 0; i < size; ++i)
          {
            ...
            {
              s = fmt::format("{}: {} ({})", i + 1, card.name_, spec);
            }
            pl->notify(s); // Print to console
        ...
        legal_actions_ = actions; // <-- Use our fully-formed actions

        // =================================================================
        // THIS IS THE CORRECTED LOGIC
        // =================================================================
        // Add finish action only if we've already met the minimum (or min is 0)
        if (ms_idx_ == ms_max_ - 1)
        { // We are about to select the last possible card
          if (ms_idx_ >= ms_min_)
          { // And we have already met the minimum
            legal_actions_.push_back(LegalAction::finish());
            if (verbose_)
            {
        ...
        { // We are between min and max-1 selections
          legal_actions_.push_back(LegalAction::finish());
          if (verbose_)
          {
            players_[player]->notify(fmt::format("{}: Finish Selection", legal_actions_.size()));
      ...
      else if (msg_ == MSG_SELECT_CHAIN)
      {
        ...
        if ((size == 0) && (spe_count == 0))
        {
          ...
          // }
          dp_ = dl_;  // prevent while-loop re-entry: dp_ was past dl_ after dp_+=8
          YGO_SetResponsei(pduel_, -1);
          return;
        }
      ...
      else if (msg_ == MSG_SELECT_YESNO)
      {
        ...
        maybe_set_action_cid(la, code, "select_yesno", msg_, desc);
        legal_actions_.push_back(la);
        if (verbose_)
        {
          auto &pl = players_[player];
      ...
      else if (msg_ == MSG_SELECT_EFFECTYN)
      {
        ...
        auto la = LegalAction::activate_spec(eff_idx, spec);
        maybe_set_action_cid(la, code_d, "select_effectyn", msg_, desc);
        legal_actions_.push_back(la);
      ...
      else if (msg_ == MSG_SELECT_OPTION)
      {
        ...
        for (int i = 0; i < size; ++i)
        {
          ...
          auto la = LegalAction::activate_spec(eff_idx, "");
          maybe_set_action_cid(la, code, "select_option", msg_, desc);
          legal_actions_.push_back(la);
          if (verbose_)
          {
            std::string s = describe_effect_label(code, eff_idx);
      ...
      else if (msg_ == MSG_SELECT_IDLECMD)
      {
        ...
        if (const char *dbg = std::getenv("YGOENV_DEBUG_IDLECMD"))
        {
          (void)dbg;
          auto dump_idle = [&](const char *label, const std::vector<IdleCardSpec> &xs)
          {
            for (int i = 0; i < xs.size(); ++i)
            {
              const auto &[code, spec, data] = xs[i];
              fmt::println(
                  stderr,
                  "[debug][MSG_SELECT_IDLECMD] {} i={} code={} spec={} data={}",
                  label, i, code, spec, data);
            }
          };
          dump_idle("summonable", summonable_);
          dump_idle("spsummon", spsummon_);
          dump_idle("repos", repos_);
          dump_idle("mset", idle_mset_);
          dump_idle("set", idle_set_);
          dump_idle("activate", idle_activate_);
        }
        bool to_bp_ = read_u8();
        bool to_ep_ = read_u8();
        read_u8(); // can_shuffle
        ...
        for (const auto &[code_t, spec, desc] : idle_activate_)
        {
          ...
          auto la = LegalAction::activate_spec(eff_idx, spec);
          maybe_set_action_cid(la, code_d, "select_idlecmd_activate", msg_, desc);
          legal_actions_.push_back(la);
          if (verbose_)
          {
            auto c = c_get_card(code);
...
struct fmt::formatter<ygopro::LegalAction> : formatter<string_view>
{
  ...
  {
    // If verbose_str_ is set, use it.
    if (!action.verbose_str_.empty())
    {
      // We just want the raw string, not quotes around it.
      // But fmt::format on a vector will add quotes.
      // Let's just return the string content.
      return format_to(ctx.out(), "{}", action.verbose_str_);
    }

    // Otherwise, fall back to the old formatting.
    std::stringstream ss;
    ss << "{";
